  ...
  $('#authForm').addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (state.role === 'patient') {
        ...
        state.token = data.token; state.user = data.patient;
        enterPatientDash();
      } else {
        ...
        state.token = data.token; state.user = data.doctor;
        enterDoctorDash();
      }
    } catch (e2) {
...
function renderTopbarRight() {
  ...
  el.innerHTML = `
    <div class="topbar-pill"><span class="dot"></span> ${state.role === 'patient' ? 'Patient' : 'Doctor'} · ${state.user.name}</div>
...
function bindDoctorHome() {
  ...
  $('#newSessionBtn').addEventListener('click', showDoctorHome);
}
